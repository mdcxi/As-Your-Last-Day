    ...
    public class AzureOverrideObjectInspector : Editor
    {
        ...
        private void OnEnable()
        {
            ...
            m_reorderableList = new ReorderableList(serializedObject, m_customPropertyList, false, true, true, true)
            {
                ...
                {
                    if (m_customPropertyList.arraySize > 0)
                    {
                        var element = m_customPropertyList.GetArrayElementAtIndex(index);
                        var expandedHeight = EditorGUI.GetPropertyHeight(element);
                        var closedHeight = EditorGUIUtility.standardVerticalSpacing + 4f;
                        if (element.isExpanded) return expandedHeight + 16f; else return expandedHeight + closedHeight;
                    }
                    else return 25f;
                },
